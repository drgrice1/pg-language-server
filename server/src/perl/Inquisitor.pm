...
sub dump_subs_from_packages {
    ...
    # Just in case we find too much stuff. Arbitrary limit of 100 subs per module, 200 fully loaded packages.  Results
    # in 10 fully loaded files in the server before we start dropping them on the ground because of the lru-cache Test
    # with these limits and then bump them up if things are working well.
    my $modLimit       = 150;
...
INSPECTOR: for my $mod (@$modpacks) {
        ...
        @$methods = sort {
            ($a =~ /^[A-Z][A-Z_]+$/) cmp($b =~ /[A-Z][A-Z_]+$/)    # Anything all UPPERCASE is at the end
        ...
        for my $name (@$methods) {
            ...
            if (my $codeRef = UNIVERSAL::can($mod, $name)) {
                ...
                # Perl mixes the two definitions anyway.
...
sub filter_modpacks {
    ...
    # Exporter:: should remove Heavy and Tiny,  Moose::Meta is removed just because it drops more than
    # 1500 things and I don't care about any of them
    my $filter_regex =
        qr/^(File::Spec::|warnings::register|lib_bs22::|Exporter::|Moose::Meta::|Class::MOP::|B::|Config::)/
