...
export const startProgress = async (connection: Connection, title: string): Promise<string | null> => {
    ...
    void connection.sendProgress(WorkDoneProgress.type, progressToken, { title, cancellable: false, kind: 'begin' });
    return progressToken;
};

export const endProgress = (connection: Connection, progressToken: string | null) => {
    if (!progressToken) return;
    void connection.sendProgress(WorkDoneProgress.type, progressToken, { kind: 'end' });
};
